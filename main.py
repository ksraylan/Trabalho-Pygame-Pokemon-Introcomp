...
def main():
    ...
    # Lista dos pokemons e seus movimentos e outras propriedades:
    # Ver a classe Pokemon localizada em pokemon.py para mais informações.
    # Picachu:
    pk_pikachu = Pokemon("Pikachu",35,55,40,90,50,50,15,random.choice(generos),
    [tipos.normal],[moves.growl, moves.thunder_shock, moves.tail_whip, moves.thunder_wave],itens_usados.copy(),
    arq.carregar_imagem_pokemon(25,True))
    # Charmander:
    pk_charmander = Pokemon("Charmander",39,52,43,65,60,50,15, random.choice(generos),
    [tipos.fire],[moves.growl, moves.scratch, moves.ember, moves.metal_claw],itens_usados.copy(),
    arq.carregar_imagem_pokemon(4,True))
    # Bulbasaur:
    pk_bulbasaur = Pokemon("Bulbasaur",45,49,49,45,65,65,15, random.choice(generos),
    [tipos.grass, tipos.poison],[moves.tackle, moves.growl, moves.leech_seed, moves.vine_whip],itens_usados.copy(),
    arq.carregar_imagem_pokemon(1,True))
    # Squirtle:
    pk_squirtle = Pokemon("Squirtle",44,48,65,43,50,64, 15,random.choice(generos),
    [tipos.water], [moves.tackle, moves.tail_whip, moves.bubble, moves.withdraw],itens_usados.copy(),
    arq.carregar_imagem_pokemon(7,True))
    # Rhyhorn:
    pk_rhyhorn = Pokemon("Rhyhorn",80,85,95,25,30,30,15,random.choice(generos),
    [tipos.ground, tipos.rock],[moves.fury_attack, moves.horn_attack, moves.stomp, moves.tail_whip],itens_usados.copy(),
    arq.carregar_imagem_pokemon(111,True))
    # Gastly:
    pk_gastly = Pokemon("Gastly",30,35,30,80,100,35,15,random.choice(generos), [tipos.ghost, tipos.poison],
    [moves.hypnosis, moves.lick, moves.spite, moves.curse],itens_usados.copy(),
    arq.carregar_imagem_pokemon(92,True))
    # Dratini:
    pk_dratini = Pokemon("Dratini",41,64,45,50,50,50,15, random.choice(generos),
    [tipos.dragon, tipos.flying], [moves.leer, moves.thunder_wave, moves.twister, moves.wrap],itens_usados.copy(),
    arq.carregar_imagem_pokemon(147,True))
    # Mewtwo:
    ...
    def desenhar_graficos():
        ...
        # Se estiver no menu de batalha:
            ...
            if sub_menu.no_submenu(sub_menu.itens):
                ...
                janela.tela.blit(arq.img_fundo_pokemon_circulo1,(janela.tamanho[0]-128*escala,48*escala))
                janela.tela.blit(arq.img_fundo_pokemon_circulo2,(4*escala,janela.tamanho[1] - 59*escala))
                ...
                if not pokemons[1].sumido:
                    ...
                    pokemons[1].deslocamento[0] * escala,72*escala - pokemons[1].imagem_frente.get_height() +
                ...
                if not pokemons[0].sumido:
                    ...
                    escala,janela.tamanho[1] - 48*escala - pokemons[0].imagem_costas.get_height() +
                ...
                # Desenhando os símbolos de gênero dos pokémons:
                # Seu pokémon:
                # Só mostrará o símbolo se ainda estiver na partida:
                if not pokemons[0].conseguiu_fugir() and not pokemons[0].foi_derrotado():
                ...
                # Pokémon inimigo:
                # Novamente, só mostrar se estiver na partida:
                if not pokemons[1].conseguiu_fugir() and not pokemons[1].foi_derrotado():
                ...
                # Agora a mesma coisa só que com o seu pokémon:
                if not pokemons[0].conseguiu_fugir() and not pokemons[0].foi_derrotado():
    ...
    def processar_logica(delta, tempo, menu, sub_menu):
        ...
        if menu.atual == menu.batalhando:
            ...
            if tempo.milisegundos >= 1000: # 1000 milisegundos = 1 segundo
    ...
    def process_turns(turnos, mensagem, tempo):
        ...
        # Só faz o movimento se ele não for None e se o pokémon não estiver bloqueado:
...
# Se o usuário está executando esse código direto no terminal:
if __name__ == "__main__":
    # Chamaremos a função "main", a função principal do jogo:
    main()